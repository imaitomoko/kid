...
<div class="content">
    ...
    <form method="GET" action="{{ route('admin.member_proxy.create', ['date' => $date]) }}" class="mb-4 d-flex align-items-center">
        <input type="text" name="child_search" placeholder="子ども名を入力" class="form-control me-2" value="{{ request('child_search') }}">
            ...
            <tbody>
                @foreach($slots as $slot)
                    <tr>
                        ...
                        <td>{{ $slot->capacity - $slot->reservations_count }}</td>
                        <td>
                            <input type="radio" name="reservation_slot_id" value="{{ $slot->id }}" required>
                        </td>
                    </tr>
                @endforeach
            </tbody>
