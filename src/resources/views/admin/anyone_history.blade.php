...
@section('css')
<link rel="stylesheet" href="{{ asset('css/admin/anyone_history.css') }}">
@endsection 
...
<div class="content">
    ...
    <div class="search">
        <form action="{{ route('admin.anyone.history') }}" method="GET" class="mb-4">
    ...
    <div class="mb-3">
        <a href="{{ route('admin.anyone.history', ['child_name' => $childName, 'month' => $prevMonth->format('Y-m')]) }}" class="before">&laquo; 前月</a>
        <span class="month">{{ $month->format('Y年m月') }}</span>
        <a href="{{ route('admin.anyone.history', ['child_name' => $childName, 'month' => $nextMonth->format('Y-m')]) }}" class="before">翌月 &raquo;</a>
    </div>
...
input.addEventListener('input', function() {
    ...
    fetch(`/admin/anyone-child-search?term=${encodeURIComponent(query)}`)
        .then(res => res.json())
        .then(data => {
            if(data.length === 0){
