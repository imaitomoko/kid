...
@section('css')
<link rel="stylesheet" href="{{ asset('css/admin/admin_schedule.css') }}">
@endsection 
...
<div class="content">
    ...
    <div class="month-navigation">
        <a href="{{ route('admin.schedule', ['year' => $current->copy()->subMonth()->year, 'month' => $current->copy()->subMonth()->month]) }}"><< 前月</a>
        <span class="month">{{ $year }}年 {{ $month }}月</span>
        <a href="{{ route('admin.schedule', ['year' => $current->copy()->addMonth()->year, 'month' => $current->copy()->addMonth()->month]) }}">翌月 >></a>
    </div>
                ...
                @for($day = 1; $day <= $daysInMonth; $day++)
                    ...
                    <td>
                        ...
                        <div class="mt-1">
                            ...
                            @else
                                <form action="{{ route('admin.schedule.show', ['date' => $date]) }}" method="GET">
                                    <button type="submit" class="btn btn-sm
                                        {{ $status === '⚪︎' ? 'btn-success' : 'btn-danger' }}">
                                        {{ $status }}
                                    </button>
                                </form>
                            @endif
